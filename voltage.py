...
traces = [ traces[0], traces[3], traces[6], traces[9], traces[12] ]
print(traces)
...
for index in range(len(cfg.amps)):
    ...
    for cell in net['cells']:
        if (cell['tags']['cellType']['amp'] == cfg.amps[index]):
            ...
            for x, trace in enumerate(traces):
                ...
                y = 4 - np.searchsorted(getattr( cfg, xvar + 's'), cell['tags']['cellType'][xvar] )
                print("plot(%s,%s)" %(x, y))
                axs[x,y].plot(t, v)
    ...
    plt.savefig("amp_%1.2f.png" %(np.round(cfg.amps[index], 2)), bbox_inches='tight')
